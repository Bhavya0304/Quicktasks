class EditableTable {
        ...
        cells.forEach((cell) => {
            if (cell.show) {
                ...
                if (cell.type == "input") {
                    ...
                    if(cell.events){
                        newCell.querySelector('input').addEventListener('keypress',cell.events);
                    }
                }
                else {
                    newCell.innerHTML = `<div class="content"> ${cell.value}</div>`;
                }
    ...
    closeEditOption = (event) => {
        ...
        if (element.classList.contains('close')) {
            ...
            var index = row.rowIndex;
            var tableInputs = Array.from(row.querySelectorAll('.input'));
            var tablecontents = Array.from(row.querySelectorAll('.content'));
            var inputIndexing = 0;
            ...
            if (isValid) {
                ...
                this.mainObj.data[index - 1].forEach((element, index) => {
                    if (element.type == "input") {
                        ...
                        element.value = contentField.outerHTML;
                        contentIndexing++;
                    }
                });
                if (row.classList.contains('default')) {
    ...
    closeAll = ()=>{
        Array.from(document.querySelectorAll('.editableable tbody tr')).forEach((row)=>{
            var index = row.rowIndex;
            var tableInputs = Array.from(row.querySelectorAll('.input'));
            var inputIndexing = 0;
            this.mainObj.data[index - 1].forEach((element, index) => {
