...
$(document).ready(function () {
    ...
    setTimer(
        ...
        timerValue,
        showAlert);
    var innerWidth = $('.mainBox').innerWidth();  
    console.log(innerWidth);  
    startAnimation(timerValue,innerWidth);
});
...
const setTimer = (htmlTag,timer,cb)=>{
    var interval = setInterval(()=>{
        if(timer < 10){
            $(htmlTag).hasClass('red') ? "": $(htmlTag).addClass('red');
        }
        if(timer < 0){
            $(htmlTag).removeClass('red');
            $(htmlTag).html("-- : --")
            clearInterval(interval);
            cb();
        }
        else{
            $(htmlTag).html(`${ parseInt(Math.floor(timer/60)).toLocaleString('en-US', {minimumIntegerDigits: 2, useGrouping:false}) } : ${ parseInt(timer%60).toLocaleString('en-US', {minimumIntegerDigits: 2, useGrouping:false}) }`);
            timer--;
        }
    },1000);
}
...
const startAnimation = (time,width)=>{
    $('.spirtes').css({left:"0px"});
    var img = $('.spirtes').innerWidth();
    $('.spirtes').animate({left:`${width - img}px`},(time*1000) + 1000,"linear");
}